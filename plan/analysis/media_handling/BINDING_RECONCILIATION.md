...
| 10 | Rule 2 violation: correlation handling deferred to "future confidence policy"; K3 admission spike dropped | **Policy bound now**: distinct-lineage counts are the only confidence input; derivation-family provenance is disclosure-only; correlation-aware adjustment is a documented alternative (not in the system). K3 admission dial restored as spike 14. Media §5/§10 |
