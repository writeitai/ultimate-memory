...
- **Phase 6, in progress** — Plane K's deterministic control plane is live: all seven closed routing-rule kinds compile to indexed coarse keys and exact SQL candidate manifests; staleness is a canonical hash over fact state, stable lineage/chunk claim coordinates, rule configuration, sidecars, child summaries, shared models, and writer versions; and compilation metadata/citations publish through a safe pending-then-commit catalog boundary. The git commit loop, deterministic fact-sheet band, writers, planner, authored-page watches/subscriptions, and belief tier remain the next work packages.
