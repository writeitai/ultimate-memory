...
def _header() -> str:
    ...
    return (
        ...
        "description: Use one configured Ultimate Memory deployment without "
...
def _recipes(*, recipes: tuple[ConsumptionRecipe, ...]) -> str:
    ...
    return (
        ...
        f"{rows}\n\n"
        "Discover the current set with `ugm query list`, `GET /recipes`, or MCP "
        "tool listing. Run one with `ugm query run <name> --arg key=value`, "
